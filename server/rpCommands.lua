        ...
        if(Config.RoleplayCommands.rpCommandCustom == true) then
            RegisterCommand(Config.RoleplayCommands.aMeName, function(source, args, raw)
                if #args <= 0 then
                    ...
                    local steam = GetPlayerName(source)
                    args = table.concat(args, ' ')
                    TriggerClientEvent('chatMessage', -1, "^8AME ~w~| ^8Anonymous User~w~", { 30, 144, 255 }, message)
        ...
        if(Config.RoleplayCommands.rpCommandCustom == true) then
            RegisterCommand("gme", function(source, args, raw)
                if #args <= 0 then
                    ...
                    PerformHttpRequest(Config.RoleplayCommands.roleplayCommandsLogging, function(err, text, headers) end, 'POST', json.encode({username = steam, content = "**GLOBAL ME:** ".. message .."", avatar_url = DISCORD_IMAGE}), { ['Content-Type'] = 'application/json' })
                end
            end)
        else
