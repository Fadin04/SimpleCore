    ...
    AddEventHandler('SimCore:call911', function(location, msg, x, y, z, name, p)
        ...
        local p = GetPlayerPed(source)
            ...
            if (Config.EmergencyCalling.callBlips == true) then
                    TriggerClientEvent('SimCore:setBlip', -1, name, x, y, z)
            end
    end)
...
if(Config.Scripts.technicalCalling == true) then
    RegisterServerEvent('SimCore:call311')
    AddEventHandler('SimCore:call311', function(location, msg, x, y, z, name, p)
        
        local pName = GetPlayerName(source)
        local p = GetPlayerPed(source)

            if (Config.EmergencyCalling.technicalCalling.callsInChat311 == true) then
                    TriggerClientEvent('chatMessage', -1, '^3----------------------------------------------', {0, 255, 238})
                    TriggerClientEvent('chatMessage', -1, '   ^*^3📞Incoming 311 Report: ', {0, 255, 238})
                    TriggerClientEvent('chatMessage', -1, '   ^*^3🧍[Caller Name]^r^7', {0, 255, 238}, pName .. " (" .. source .. ")")
                    TriggerClientEvent('chatMessage', -1, '   ^*^3🗺️[Location]^r^7', {0, 255, 238}, location)
                    TriggerClientEvent('chatMessage', -1, '   ^*^3⚠️[Report]^r^7', {0, 255, 238}, msg)
                    TriggerClientEvent('chatMessage', -1, '^3----------------------------------------------', {0, 255, 238})
                end
                sendDiscord('311 Call Logger \n', '**Caller Name: **' .. pName .. ' (' .. source .. ') \n**  Location: **' .. location .. '\n**  Report: **' .. msg)
            
            if (Config.EmergencyCalling.technicalCalling.callBlips311 == true) then
                    TriggerClientEvent('SimCore:set311Blip', -1, name, x, y, z)
            end
    end)
    ...
    function sendDiscord(name, message)
        ...
        PerformHttpRequest(Config.EmergencyCalling.technicalCalling.loggingWebhook, function(err, text, headers) end, 'POST', json.encode({username = name, embeds = content}), { ['Content-Type'] = 'application/json' })
    end
end
    ...
    RegisterCommand('fix', function(source, args, rawCommand)
        if (Config.VehicleControl.vehicleMaintenanceScripts.fixPermissions == true) then 
            if IsPlayerAceAllowed(source, "vehicle.fix") then
                TriggerClientEvent("SimCore:fixVehicle", -1)
            else
                TriggerClientEvent("SimCore:nofPerms", -1)
            end
        else
            TriggerClientEvent("SimCore:fixVehicle", -1)
        end
    ...
    RegisterCommand('clean', function(source, args, rawCommand)
        if (Config.VehicleControl.vehicleMaintenanceScripts.cleanPermissions == true) then 
            if IsPlayerAceAllowed(source, "vehicle.clean") then
                TriggerClientEvent("SimCore:cleanVehicle", -1)
            else
                TriggerClientEvent("SimCore:nocPerms", -1)
            end
        else
            TriggerClientEvent("SimCore:cleanVehicle", -1)
        end
