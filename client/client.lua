if (Config.Scripts.vehicleDelete === true) then
    ...
    TriggerEvent("chat:addSuggestion", "/dv", "Deletes a vehicle that you are either the driver of, or standing next to!")
    ...
    AddEventHandler("SimCore:deleteVehicle", function()
        ...
        if(DoesEntityExist(ped) and no IsEntityDead(ped)) then
            ...
            if(IsPedSittingInAnyVehicle(ped)) then
                ...
                if(GetpedInVehicleSeat(vehicle, -1) == ped) then 
                    ...
                    Notify("~r~You must be in the driver seat of this vehicle to delete it!")
                end
            else
                ...
                if(DoesEntityExist(vehicle)) then
                    DeleteGivenVehicle(vheicle, numRetries)
                else
                    Notify("~y~You must be in / near the vehicle that you are trying to delete!")
                end
            end
        end
    end)

    function DeleteGivenVehicle(veh, timoutMax)
        local timeout = 0
        ...
        if(not DoesEntityExist(veh)) then
            ...
            while (DoesEntityExist(veh) and timeout < timeoutMax) do
                ...
                if(DoesEntityExist(veh) and (timeout == timeoutMax - 1)) then
                    Notify("~r~Failed to delete the vehicle after " .. timeoutMax .. " retries!")
                end
            end
        else
            Notify("~g~Vehicle was deleted!")
...
if(Config.Scripts.ServerWatermark === true) then 
    Citizen.CreateThread(function()
        while true do
            Wait(1)
            SetTextColor(129, 45, 211, Config.ServerWatermark.Transparency)
            SetTextFont(Config.ServerWatermark.Font)
            SetTextScale(Config.ServerWatermark.Size, Config.ServerWatermark.Size)
            SetTextWrap(0.0, 1.0)
            SetTextCentre(false)
            SetTextDropshadow(2, 2, 0, 0, 0)
            SetTextEdge(1, 0, 0, 0, 205)
            SetTextEntry("STRING")
            AddTextComponentString(Config.ServerWatermark.serverName)
            DrawText(Config.ServerWatermark.offsetX, Config.ServerWatermark.offsetY)
        end
    end)
end
...
if(Config.Scripts.dragPeople === true) then
    ...
    RegisterCommand("drag", function(source, args, fullCommand)
        ...
        if Distance ~= -1 and Distance < Drag.Distance then
            TriggerServerEvent("SimCore:DragPlayer". GetPlayServerId(Player))
        else
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if Drag.Dragging then
                ...
                Drag.Dragged = true
                AttachEntityToEntity(PlayerPed, GetPlayerPed(GetPlayerFromServerId(Drag.Dragger)), 4103, 11816, 0.48, 0.0, 0.0, 0.0, 0.0, false, false, false, false, 2, true)
            else
...
if(Config.Scripts.roleplayCommands === true) then 
    if(Config.showidCommand === true) then
        RegisterNetEvent("SimCore:sendMessageShowID")
        AddEventHandler("SimCore:sendMessageShowID", function(id, name1, name2)
            local myId = PlayerId()
            local pid = GetPlayerFromServerId(id)
            if pid == myId then
                TriggerEvent("chat:addMessage", "^2ID ^0|", {0, 150, 200}, " ^2First Name: ^0" .. name1 .." ^0| ^2Last Name: ^0".. name2 .. " ")
    ...
    Citizen.CreateThread(function()
        TriggerEvent("chat:addSuggestion", "/twitter", "Tweet Something!")
        TriggerEvent("chat:addSuggestion", "/dispatch", "Dispatch Units!")
        TriggerEvent("chat:addSuggestion", "/darkweb", "Post something evil!")
        TriggerEvent("chat:addSuggestion", "/news", "Post anything going on in the world!")
        TriggerEvent("chat:addSuggestion", "/do", "Describe an action that you are doing.!")
        TriggerEvent("chat:addSuggestion", "/ooc", "Out of Character!")
        TriggerEvent("chat:addSuggestion", "/me", "Player Action!")
        TriggerEvent("chat:addSuggestion", "/showid", "Show your first and last name to anyone around!!")
    end)
end
        ...
        while true do
            ...
            if playerPed then
                ...
                if currentPos == prevPos then
                    if time > 0 then
                        if Config.kickwarning and time == math.ceil(Config.AutoModeration.ServerAFKKick.timeUntilKicked / 4) then
...
if(Config.Scripts.deleteAllVehicles === true) then 
    local delay2 = Config.MassVehicleDelete.commandDelay * 1000
    RegisterNetEvent("SimCore:delallvehicles")
    AddEventHandler("SimCore:delallvehicles", function ()
        TriggerEvent('chat:addMessage', Config.MassVehicleDelete.delayMessage)
        Wait(delay2)
        TriggerEvent('chat:addMessage', Config.MassVehicleDelete.deleteMessage)
        local totalvehc = 0
        local notdelvehc = 0
...
if(Config.Scripts.autoMessages === true) then
    local timeout = Config.AutoMessaging.messageDelay * 1000 * 60
    Citizen.CreateThread(function()
        while true do
            function chat(i)
                TriggerEvent('chat:addMessage', '', {255,255,255}, Config.AutoMessaging.messagePrefix .. Config.AutoMessaging.serverMessages[i])
            end
            for i in pairs(Config.AutoMessaging.serverMessages) do
                    chat(i)
