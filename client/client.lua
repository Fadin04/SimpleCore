...
if(Config.Scripts.roleplayCommands == true) then 
    ...
    Citizen.CreateThread(function()
        TriggerEvent("chat:addSuggestion", "/twitter", "Tweet Something!", {
            {name = "Twitter", help = "Post something interesting on twitter!"}
        })
        ...
        TriggerEvent("chat:addSuggestion", "/darkweb", "Post something evil!", {
            {name = "Darkweb", help = "Do all of your evil doings on the darkweb!"}
        })
        ...
        TriggerEvent("chat:addSuggestion", "/do", "Describe an action that you are doing.!", {
            {name = "Do", help = "Show others what your rp character is doing!"}
        })
        TriggerEvent("chat:addSuggestion", "/ooc", "Out of Character!", {
            {name = "OOC", help = "Talk to other members in the server whilst being out of character!"}
        })
        TriggerEvent("chat:addSuggestion", "/me", "Player Action!", {
            {name = "Me", help = "Share an action that you are doing with other players!"}
        })
        ...
        TriggerEvent('chat:addSuggestion', "gme", "Share a player action globally!", {
            {name = "GME", help = "Share what you're character is doing to everyone!"}
        })
    end)
end
...
if(Config.Scripts.emergencyCalling == true) then
    if (Config.EmergencyCalling.chatSuggestion == true) then
        ...
        RegisterNetEvent('SimCore:setBlip')
        AddEventHandler('SimCore:setBlip', function(name, x, y, z)
...
if(Config.Scripts.technicalCalling == true) then
    if (Config.EmergencyCalling.technicalCalling.chatSuggestion311 == true) then
        TriggerEvent("chat:addSuggestion", "/311", "Call for road service!", {
            { name = "Report", help = "Enter your report here!"}
        })
    end
        ...
        local msg = table.concat(args, ' ')
            ...
            if args[1] ~= nil then
                TriggerServerEvent('SimCore:call311', location, msg, x, y, z, name)
            end
        end)
        
        RegisterNetEvent('SimCore:set311Blip')
        AddEventHandler('SimCore:set311Blip', function(name, x, y, z)
        
        blip = nil
        blips = {}
        
        local blip = AddBlipForRadius(x, y, z, Config.EmergencyCalling.technicalCalling.blipRadius311)
        ...
        local blip1 = AddBlipForCoord(x, y, z)
            ...
            EndTextCommandSetBlipName(blip1)
            table.insert(blips, blip1)
            Wait(Config.EmergencyCalling.technicalCalling.blipTime311 * 1000)
            for i, blip1 in pairs(blips) do 
                RemoveBlip(blip)
                RemoveBlip(blip1)
